  ...
  methods: {
    ...
    async signup(user_name, password, is_dev, email, company) {
      ...
      await me.$axios
        ...
        .then(async resp => {
          me.swAlert("Success", "Successfully created user", "success", () => {
            me.$nuxt.$router.push(me.localePath("/login"));
          });
        })
        .catch(error => {
          me.sweetsAlert("Sign up failed", error, "error");
        });
    },
      ...
      await me.$axios
        ...
        .then(async resp => {
          ...
          me.setCookie("email", resp.data.email.toString(), 1),
            me.setCookie("isAdmin", resp.data.isAdmin.toString(), 1),
            me.$nuxt.$router.push(me.localePath("/"));
        })
        .catch(error => {
          if (this.$axios.isCancel(error)) {
          } else {
            me.sweetsAlert("Login failed", error, "error");
          }
        });
    }
  }
